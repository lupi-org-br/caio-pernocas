...
- **Contextual Grouping:** Instead of making something like `wifi_connect`, `wifi_disconnect`, `wifi_reconnect`, `wifi_scan`, etc., group related code together in a folder `wifi`, with a `connect.lua`, `disconnect.lua`, `reconnect.lua`, `scan.lua`, etc.
