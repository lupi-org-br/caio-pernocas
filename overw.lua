...
function make_overworld()
    ...
    local function tile_at(x, y)
        local w = data.metadata.width
        local h = data.metadata.height
        if x < 1 or x > w or y < 1 or y > h then return nil end
        return data.FG.FG[(y - 1) * w + x]
    end
    ...
    return {
        ...
        draw = function(current_frame)
            ...
            ui.camera(camx, camy)
            ui.rectfill(35, 35, 480 - 35 *2, 270 - 35 * 2, kColors.purple_dark)
            ui.clip(40, 40, 480 - 40 *2, 270 - 40 * 2)
