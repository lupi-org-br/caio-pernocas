...
function StateRoll.update(ctx, frame)
    ...
    if ctx.roll_direction > 0 then
        wall_ahead = ctx.map_ref.colides(ctx.position.x + ctx.size.w - 4, ctx.position.y + ctx.size.h - 1, kColisionType.right)
    else
        wall_ahead = ctx.map_ref.colides(ctx.position.x + 4, ctx.position.y + ctx.size.h - 1, kColisionType.left)
    end
    ...
    ctx.velocity.x = (ctx.roll_direction * 0.2) * kPlayerRollSpeed
    return nil
end
